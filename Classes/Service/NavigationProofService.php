...
final class NavigationProofService
{
    ...
    public function authorize(ServerRequestInterface $request, array $settings, bool $protected, callable $verify): ?string
    {
        ...
        return $this->update($identifier, function (&$state) use ($identifier, $scope, $fingerprint, $ajaxScope, $request, $settings, $protected, $token, $verify): ?string {
            if (is_array($state)) {
                if ($state['scope'] !== $scope || $fingerprint === null || (isset($state['ajaxScope']) && $ajaxScope === null)) {
                    return null;
                }
                if (!isset($state['requests'][$fingerprint]) && in_array($fingerprint, $state['redirects'], true)) {
                    $state['requests'][$fingerprint] = true;
                } elseif ($ajaxScope !== null && ($state['ajaxScope'] ?? null) === $ajaxScope && $state['ajaxRemaining'] > 0) {
                    // A view can load several datasets; every request uses one slot, including repeats.
                    --$state['ajaxRemaining'];
                    $state['requests'][$fingerprint] = true;
                } elseif (!isset($state['ajaxScope']) && $request->getMethod() === 'GET' && isset($state['requests'][$fingerprint]) && !$state['duplicateUsed']) {
                ...
                $state = [
                    ...
                    'requests' => $fingerprint === null ? [] : [$fingerprint => true],
                    'redirects' => [],
                    'duplicateUsed' => false,
                    'ajaxScope' => $ajaxScope,
    ...
    {
        return strtolower($uri->getScheme() . '://' . $uri->getHost()) . ':' . ($uri->getPort() ?? ($uri->getScheme() === 'https' ? 443 : 80));
    }
